...
from svn.errors import SVNErrorCode, SVNError
import re
...
class CommonClient(object):
    PARSE_ERROR_CODE = re.compile(r"^svn: E(\d{5,}):", re.MULTILINE)
    ...
    def run_command(self, subcommand, args, success_code=0,
        ...
        if r != success_code:
            error_codes = self.PARSE_ERROR_CODE.findall(stdout.decode())
            SVNErrorClasses = []
            for m in error_codes:
                try:
                    SVNErrorClasses.append(SVNErrorCode(int(m)).errcls)
                except:
                    pass

            if not SVNErrorClasses:
                SVNErrorClasses.append(SVNError)

            for ec in SVNErrorClasses:
                raise ec(stdout.decode(), p.returncode, ' '.join(cmd))
    ...
    def info(self, rel_path=None):
        ...
        result = self.run_command('info', ['--xml', full_url_or_path],
                                  combine=True)
        ...
        info = {
            ...
            'relative_url': relative_url.text if relative_url is not None and len(
                relative_url.text) else None,
            ...
            'wc-info/wcroot-abspath': wcroot_abspath.text if wcroot_abspath is not None and len(
                wcroot_abspath.text) else None,
            'wc-info/schedule': wcinfo_schedule.text if wcinfo_schedule is not None and len(
                wcinfo_schedule.text) else None,
            'wc-info/depth': wcinfo_depth.text if wcinfo_depth is not None and len(
                wcinfo_depth.text) else None,
            'commit/author': author.text if author is not None and len(
                author.text) else None,
